...
def makeChromosone():
	...
	for state in range(statevals+1):
		for cell in range(cellvals+1):
			dst = (randint(0,statevals)-state,randint(0,cellvals),2*randint(0,1)-1)
			prog[(state,cell)] = dst
	return prog
	...
	if fitness > bestfit or (bestprog is not None and fitness == bestfit and len(seen) < len(bestprog)):
		...
		for k in ks:
			...
			if k[1] != v[1]:
				if k[1] == 5:
					if v[1] == 0:
						...
						actions.append('replace the hotel with %d houses' % v[1])
				elif k[1] == 0:
					if v[1] == 5:
			...
			if len(actions) == 0:
				...
				row.append('"%s, %s and %s"' % (actions[0],actions[1],actions[2]))
			f.write(','.join(row))
			f.write('\n')
		f.close()
